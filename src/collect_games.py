...
from config import GAMES_FILE, RAW_GAMES_DIR
...
def collect_games(seasons: list[str], season_type: str = "Regular Season") -> pd.DataFrame:
    ...
    for season in seasons:
        ...
        raw_path = RAW_GAMES_DIR / f"league_game_log_{season.replace('-', '_')}.csv"
    ...
    print(f"Saved {len(games)} unique games to {GAMES_FILE}")
    return games
